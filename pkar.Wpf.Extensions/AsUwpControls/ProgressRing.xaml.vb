...
Public Class ProgressRing
    ...
    Public Shared ReadOnly IsActiveProperty As DependencyProperty =
        DependencyProperty.Register("IsActive", GetType(Boolean), GetType(ProgressRing), New PropertyMetadata(False))
    ...
    Private Sub SetItemPosition(item As DependencyObject, index As Integer)
        ...
        'Debug.WriteLine($"SetItemPosition(index={index}, srodek={srodekX}, kat={kat}, xOffset={xOffset}, yOffset = {yOffset}, item diameter={ItemDiameter}")

        item.SetValue(Canvas.LeftProperty, srodekX + xOffset + xOffset.Sign * ItemDiameter / 2)
        item.SetValue(Canvas.TopProperty, srodekY + yOffset + yOffset.Sign * ItemDiameter / 2)
        item.SetValue(WidthProperty, ItemDiameter)
        item.SetValue(HeightProperty, ItemDiameter)
    End Sub
    ...
    Protected Overrides Sub OnRenderSizeChanged(sizeInfo As SizeChangedInfo)
        ...
        ItemDiameter = Math.Max(Diameter / 20, 5)
        ' kropka ma być w połowie wielkości pola/controlki
        ItemPositionRadius = (Diameter - ItemDiameter) / 2.0
